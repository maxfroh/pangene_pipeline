...
if mamba env list | grep -q "^${PANGENE_PIPELINE_ENV_NAME}\s"; then
    ...
    ENV_LOC=$(mamba env list | grep -E "^\s*${PANGENE_PIPELINE_ENV_NAME}\s+" | awk -v env_name="$PANGENE_PIPELINE_ENV_NAME" '{ if ($2 ~ env_name) print $2; else print $3 }' | sed 's/$/\/bin/')
