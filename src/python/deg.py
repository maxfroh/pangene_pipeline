...
class DEG():
    def __init__(self, mgr: dict[str, str | RunManager]):
    ...
    def clean_reads(self, fq_in: Path, fq_out: Path):
        for i in range(len(self.mgr.samples)):
            ...
            fq_out = self.mgr.tmp_dir / \
                self.mgr.append_filename(fq_in, "trimmed").name
            cmds = ["java", "-jar", "trimmomatic-0.40.jar",
                    "SE", fq_in, fq_out, "ILLUMINA?", "..."]
    ...
    def kallisto_quantify(self, idx_file: Path):
        ...
        for sample, sample_name in self.mgr.sample_names.items():
            cmds = ["kallisto", "quant", "-i", self.mgr.kallisto_dir / idx_file, "-t", self.mgr.p, "--single", "-l", self.mgr.frag_length_mean,
                    "-s", self.mgr.frag_length_std, "-o", self.mgr.kallisto_dir / f"{sample_name}", self.mgr.sample_dir / sample]
    ...
    def make_condition_table(self):
        ...
        for sample_name in self.mgr.sample_names.values():
    ...
    def perform_deseq_analysis(self):
        cmds = ["Rscript", "./src/R/deseq.R", self.mgr.kallisto_dir, self.column_data_file, self.mgr.annotation_file,
                self.mgr.tmp_dir / f"deseq_results_{self.reference}.tsv", self.mgr.alpha, self.mgr.l2FC_thresh, *self.mgr.sample_names.values()]
